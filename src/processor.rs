...
use std::{fs::File as FsFile, io::{ErrorKind, Read, Write}, path::{Path, PathBuf}};
...
pub fn remove_comments(content: String, file: File) -> Result<File, std::io::Error> {
    ...
    // Initialize `new_file_path` outside the if block to make it accessible later
    let new_file_path;
    if is_modified {
        ...
        new_file_path = dir_path.join(&new_file_name); // Assign value to `new_file_path` here
        ...
        // Assign a default value to `new_file_path` if not modified
        new_file_path = PathBuf::from(&file.path);
    }

    // Now `new_file_path` is accessible here
